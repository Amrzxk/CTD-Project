...
const SEVERITY_OPTS: { label: string; key: AlertFilters['severity']; cls: string }[] = [
  { label: 'High',   key: 'high',   cls: 'text-[#ff3366] border-[#ff3366]/40' },
  { label: 'Medium', key: 'medium', cls: 'text-yellow-400 border-yellow-400/40' },
  { label: 'Low',    key: 'low',    cls: 'text-[#00ccff] border-[#00ccff]/40' },
];
...
export function AlertFilterBar({ value, onChange, searchInputRef }: Props) {
  ...
  return (
    <div className="mb-4 space-y-2">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <div className="relative flex-1 min-w-[240px]">
          ...
          <input
            ...
            className="w-full bg-[#0f1825]/80 border border-[#1a2540] rounded-md pl-8 pr-7 py-1.5 text-xs text-gray-200 placeholder-gray-600 focus:outline-none focus:border-[#00ccff]/60 font-mono"
            spellCheck={false}
          />
          {qLocal && (
            <button
            ...
            return (
              <button
                ...
                className={`px-2 py-1 rounded border text-[11px] uppercase tracking-wide font-semibold transition-colors ${
                  active ? `${s.cls} bg-[#0f1825]/80` : 'border-[#1a2540] text-gray-500 hover:text-gray-300'
                }`}
              >
                {s.label}
              </button>
        ...
        <label className="flex items-center gap-1 text-[11px] text-gray-500">
          ...
          <select
            ...
            className="bg-[#0f1825]/80 border border-[#1a2540] rounded-md py-1 px-1.5 text-[11px] text-gray-200 focus:outline-none focus:border-[#00ccff]/60"
          >
            {SORT_OPTS.map((o, i) => (
        ...
        <button
          ...
          className={`px-2 py-1 rounded border text-[11px] inline-flex items-center gap-1 ${
            expanded
              ? 'bg-[#1a2540]/60 border-[#1a2540] text-gray-200'
        ...
        {hasAny && (
          <button
            type="button"
            onClick={clearAll}
            className="px-2 py-1 rounded text-[11px] text-gray-400 hover:text-white inline-flex items-center gap-1"
      ...
      {expanded && (
        <div className="flex items-center gap-2 flex-wrap text-[11px] text-gray-400 pl-1">
          <span className="text-[10px] uppercase tracking-wide text-gray-600">CIDR</span>
          <input
            type="text"
            ...
            className="bg-[#0f1825]/80 border border-[#1a2540] rounded px-2 py-1 text-[11px] font-mono text-gray-200 focus:outline-none focus:border-[#00ccff]/60 w-44"
            spellCheck={false}
          />
          <input
            type="text"
            ...
            className="bg-[#0f1825]/80 border border-[#1a2540] rounded px-2 py-1 text-[11px] font-mono text-gray-200 focus:outline-none focus:border-[#00ccff]/60 w-52"
            spellCheck={false}
          />
          <span className="text-[10px] uppercase tracking-wide text-gray-600 ml-2">Dst port</span>
          <input
            type="number"
            min={0}
            max={65535}
            ...
            className="bg-[#0f1825]/80 border border-[#1a2540] rounded px-2 py-1 text-[11px] font-mono text-gray-200 focus:outline-none focus:border-[#00ccff]/60 w-20"
          />
