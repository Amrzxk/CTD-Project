...
function FragmentRow({
  ...
  return (
    <>
      <div className="text-[11px] font-mono text-gray-400 py-1 pr-2 whitespace-nowrap">
        {label}
      </div>
      {families.map((f) => {
        const c = counts[f] ?? 0;
        ...
        return (
          <div
            key={f}
            className="h-7 rounded-sm flex items-center justify-center text-[10px] font-mono transition hover:ring-1 hover:ring-white/40"
            style={{
              background: c > 0
...
export default function AnalyticsPage() {
  ...
  if (!analytics || flowCount === 0) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#0b0f1a] via-[#111a2e] to-[#060a14] flex flex-col items-center justify-center p-4">
        <div className="text-center bg-[#0f1825]/70 p-8 rounded-2xl border border-[#1a2540] backdrop-blur max-w-lg shadow-2xl">
          <Activity className="w-16 h-16 text-[#9ca3af] mx-auto mb-4 opacity-70" />
          {inconsistent ? (
            <>
              <h2 className="text-2xl font-bold text-[#ff3366] mb-3">
                Backend returned an inconsistent response
              </h2>
              <p className="text-gray-300 mb-4">
                You're on <span className="font-mono text-[#00ccff]">all</span>, the store has{' '}
                <span className="font-mono text-[#00ccff]">
                  {(analyticsStoreTotal || rawStoreTotal || 0).toLocaleString()}
                </span>{' '}
                stored predictions, but the analytics endpoint reports{' '}
                <span className="font-mono text-[#ff3366]">0 flows</span>. This is impossible per
                the backend code — almost always a stale process or HTTP cache.
              </p>
              <div className="text-left text-[12px] bg-[#1a2540]/40 border border-[#1a2540] rounded-lg p-3 mb-4 space-y-1">
                <p className="text-gray-300 font-semibold mb-1">Recovery steps:</p>
                <ol className="list-decimal list-inside text-gray-400 space-y-0.5">
                  ...
                  <li>
                    ...
                    <span className="font-mono text-[#ffaa00]">netstat -ano | findstr :8000</span>.
                  ...
                  <li>
                    ...
                    <a
                      ...
                      className="text-[#00ccff] underline underline-offset-2"
                    >
                      /_debug/store
                    </a>{' '}
                ...
                <p className="text-gray-500 text-[11px] pt-2">
                  ...
                  · totalFlows ={' '}
                  <span className="font-mono text-[#ff3366]">{flowCount}</span>
                </p>
              </div>
              <div className="flex items-center justify-center gap-2">
                <button
                  ...
                  className="px-3 py-1.5 rounded-md border border-[#00ccff]/40 text-[#00ccff] text-xs hover:bg-[#00ccff]/10 inline-flex items-center gap-1"
                >
                ...
                <button
                  type="button"
                  onClick={handleReset}
                  className="px-3 py-1.5 rounded-md border border-[#ff3366]/40 text-[#ff3366] text-xs hover:bg-[#ff3366]/10 inline-flex items-center gap-1"
            ...
            <>
              <h2 className="text-2xl font-bold text-white mb-3">No flows in this time window</h2>
              <p className="text-gray-400 mb-4">
                ...
                <span className="font-mono text-[#00ccff]">{analytics?.timeRangeApplied ?? timeRange}</span>.
              ...
              <div className="flex items-center justify-center gap-2 mb-3">
                <button
                  ...
                  className="px-3 py-1.5 rounded-md border border-[#00ff88]/40 text-[#00ff88] text-xs hover:bg-[#00ff88]/10"
                >
                  Switch to All
                </button>
            ...
            <>
              ...
              <div className="text-left text-[12px] bg-[#1a2540]/40 border border-[#1a2540] rounded-lg p-3 mb-4 space-y-1">
                <p className="text-gray-300 font-semibold">Common causes:</p>
                <ul className="list-disc list-inside text-gray-400 space-y-0.5">
                  ...
                  <li>
                    <span className="font-mono text-[#ffaa00]">uvicorn --reload</span> auto-restarts
              ...
              {contradiction && (
                <div className="text-[12px] bg-[#ff3366]/10 border border-[#ff3366]/30 rounded-lg p-3 mb-4 text-left">
                  <p className="text-[#ff3366] font-semibold mb-1">⚠ Contradiction detected</p>
  ...
  const TrendDelta = ({
    ...
    if (prior === 0) {
      return (
        <span className="text-[10px] font-mono inline-flex items-center gap-0.5 text-[#ffaa00]">
    ...
    if (Math.abs(delta) < 1) {
      return <span className="text-[10px] font-mono text-gray-500">≈</span>;
    }
    const up = delta > 0;
    // up + goodWhenDown → bad (red). up + !goodWhenDown → good (green).
    const isBad = up === goodWhenDown;
    const color = isBad ? 'text-[#ff3366]' : 'text-[#00ff88]';
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0b0f1a] via-[#111a2e] to-[#060a14] py-12">
      <div className="container mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            ...
            <div>
              ...
              <p className="text-gray-400">
                ...
                {analytics?.timeRangeApplied && analytics?.totalFlows !== undefined && (
                  <> · <span className="text-gray-300 font-mono">{analytics.totalFlows.toLocaleString()}</span> flows
                  in window <span className="text-gray-300 font-mono">{analytics.timeRangeApplied}</span></>
                )}
                {lastLoaded && (
                  <> · loaded <span className="text-gray-300 font-mono">{lastLoaded.toLocaleTimeString()}</span></>
                )}
              </p>
          ...
          {autoWidenedFrom && (
            <div className="mb-6 flex items-center justify-between gap-3 px-4 py-3 rounded-lg border border-[#00ccff]/30 bg-[#00ccff]/5">
              <div className="flex items-center gap-2 text-sm text-[#00ccff]">
            ...
            return (
              <div className="grid md:grid-cols-4 gap-6 mb-8">
                <Card className="bg-[#0f1825]/70 border-[#1a2540] backdrop-blur">
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between mb-2">
                      <Clock className="w-8 h-8 text-[#00ccff]" />
                      <span className="text-xs text-[#00ccff] font-semibold uppercase tracking-wide">MTTA</span>
                    </div>
                    ...
                    <p className="text-sm text-gray-400">
                      ...
                      {k.mttaSampleSize !== undefined && (
                        <span className="text-gray-600"> · n={k.mttaSampleSize}</span>
                      )}
                    </p>
                  </CardContent>
                </Card>
                <Card className="bg-[#0f1825]/70 border-[#1a2540] backdrop-blur">
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between mb-2">
                      <Inbox className="w-8 h-8 text-[#ff3366]" />
                      <span className="text-xs text-[#ff3366] font-semibold uppercase tracking-wide">Backlog</span>
                    </div>
                    <p className="text-3xl font-bold text-white mb-1">
                      {k.backlog.toLocaleString()}
                    </p>
                    <p className="text-sm text-gray-400">Unacked alerts in window</p>
                  </CardContent>
                </Card>
                <Card className="bg-[#0f1825]/70 border-[#1a2540] backdrop-blur">
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-xs text-[#ffaa00] font-semibold uppercase tracking-wide">Oldest unacked</span>
                    </div>
                    <p className="text-3xl font-bold text-white mb-1">
                      {fmtSeconds(k.oldestUnackedAgeSeconds)}
                    </p>
                    <p className="text-sm text-gray-400">Age of stalest queue entry</p>
                  </CardContent>
                </Card>
                <Card className="bg-[#0f1825]/70 border-[#1a2540] backdrop-blur">
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-xs text-[#00ff88] font-semibold uppercase tracking-wide">Closed</span>
                    </div>
                    <p className="text-3xl font-bold text-white mb-1">
                      {k.ackedInWindow.toLocaleString()}
                    </p>
                    <p className="text-sm text-gray-400">Acked / escalated / dismissed</p>
                  </CardContent>
                </Card>
              </div>
            );
          ...
          <div className="grid md:grid-cols-4 gap-6 mb-10">
            <Card className="bg-gradient-to-br from-[#ff3366]/15 to-[#ff3366]/5 border-[#ff3366]/40 backdrop-blur">
              <CardContent className="p-6">
                ...
                <div className="flex items-center gap-2 mb-1">
                  <p className="text-3xl font-bold text-white">{analytics.severityCounts.high}</p>
            ...
            <Card className="bg-gradient-to-br from-yellow-500/15 to-yellow-600/5 border-yellow-500/40 backdrop-blur">
              <CardContent className="p-6">
                ...
                <div className="flex items-center gap-2 mb-1">
                  <p className="text-3xl font-bold text-white">{analytics.severityCounts.medium}</p>
            ...
            <Card className="bg-gradient-to-br from-[#00ccff]/15 to-[#00ccff]/5 border-[#00ccff]/40 backdrop-blur">
              <CardContent className="p-6">
                ...
                <div className="flex items-center gap-2 mb-1">
                  <p className="text-3xl font-bold text-white">{analytics.severityCounts.low}</p>
                  <TrendDelta current={analytics.severityCounts.low} prior={analytics.prior?.severityCounts.low} />
                </div>
                <p className="text-sm text-gray-400">Low / Suspicious</p>
              </CardContent>
            </Card>
            <Card className="bg-gradient-to-br from-[#00ff88]/15 to-[#00ff88]/5 border-[#00ff88]/40 backdrop-blur">
              <CardContent className="p-6">
                ...
                <p className="text-3xl font-bold text-white mb-1">{threatPercentage}%</p>
          ...
          <h2 className="text-sm font-semibold text-gray-300 uppercase tracking-wide mb-3 flex items-center gap-2">
            <ShieldAlert className="w-4 h-4 text-[#00ccff]" /> Detection Breakdown
          </h2>
          <div className="grid lg:grid-cols-2 gap-8 mb-10">
            <Card className="bg-[#0f1825]/70 border-[#1a2540] backdrop-blur">
              ...
              <CardContent>
                {analytics?.verdictBreakdown && analytics.verdictBreakdown.length > 0 ? (
                  <ResponsiveContainer width="100%" height={300}>
                    <PieChart>
                      <Pie
                        ...
                        dataKey="value" strokeWidth={2} stroke="#0f1825"
                        cursor="pointer"
                  ...
                  <div className="text-center py-12 text-gray-500 text-sm">No verdict data.</div>
                )}
              </CardContent>
            </Card>
            ...
            <Card className="bg-[#0f1825]/70 border-[#1a2540] backdrop-blur">
              <CardHeader>
                <CardTitle className="text-white flex items-center gap-2">
                  <Layers className="w-5 h-5 text-[#00ccff]" />
              ...
              <CardContent>
                {analytics?.familyBreakdown && analytics.familyBreakdown.length > 0 ? (
                  ...
                  <div className="text-center py-12 text-gray-500 text-sm">No family data.</div>
                )}
              </CardContent>
            </Card>
          </div>
          ...
          {analytics?.severityByFamily && analytics.severityByFamily.length > 0 && (
            <Card className="bg-[#0f1825]/70 border-[#1a2540] backdrop-blur mb-10">
              <CardHeader>
                <CardTitle className="text-white flex items-center gap-2">
                  <BarChart3 className="w-5 h-5 text-[#00ccff]" />
                  Severity by Family
                </CardTitle>
                <p className="text-xs text-gray-500 mt-1">
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={320}>
                  <BarChart data={analytics.severityByFamily} margin={{ left: 0, right: 20, top: 10, bottom: 0 }}>
                    ...
                    <Bar dataKey="high" stackId="sev" fill="#ff3366" name="High" radius={[0, 0, 0, 0]} />
                    <Bar dataKey="medium" stackId="sev" fill="#facc15" name="Medium" />
                    <Bar dataKey="low" stackId="sev" fill="#00ccff" name="Low" radius={[6, 6, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-[#1a2540] text-left text-xs text-gray-500 uppercase tracking-wide">
                        ...
                        return analytics.leafBreakdown!.map((l) => {
                          ...
                          return (
                            <tr
                              ...
                              className="border-b border-[#1a2540]/40 hover:bg-[#1a2540]/30 cursor-pointer"
                              title={`Click to filter Alerts by ${l.leaf}`}
                            >
                              <td className="py-2 pr-4 font-mono text-gray-200">{l.leaf}</td>
                              <td className="py-2 pr-4 text-gray-400 font-mono text-xs">{l.family}</td>
                              <td className="py-2 pr-4 text-right font-mono text-gray-200">{l.count.toLocaleString()}</td>
                              <td className="py-2 pr-4 text-right font-mono text-gray-400 text-xs">{pct.toFixed(1)}%</td>
                              <td className="py-2 pr-4">
                                <div className="h-2 rounded-full bg-[#1a2540]/60 overflow-hidden">
          ...
          {/* ───── SECTION 3: TARGETS ───── */}
          <h2 className="text-sm font-semibold text-gray-300 uppercase tracking-wide mb-3 flex items-center gap-2">
            <Crosshair className="w-4 h-4 text-[#ff3366]" /> Attack Targets
          </h2>
          <div className="grid lg:grid-cols-3 gap-8 mb-10">
            ...
            <Card className="bg-[#0f1825]/70 border-[#1a2540] backdrop-blur">
              ...
              <CardContent>
                {analytics.topMaliciousIPs.length > 0 ? (
                  ...
                  <div className="text-center py-12 text-gray-500 text-sm">No malicious source IPs.</div>
                )}
              </CardContent>
            </Card>
                ...
                {analytics.topDestinations && analytics.topDestinations.length > 0 ? (
                  <ResponsiveContainer width="100%" height={260}>
                    <BarChart data={analytics.topDestinations} layout="vertical" margin={{ left: 10, right: 20 }}>
                      ...
                      <YAxis type="category" dataKey="ip" stroke="#9ca3af" style={{ fontSize: '11px', fontFamily: 'monospace' }} width={110} />
                      <Tooltip content={<DarkTooltip />} />
                      <Bar dataKey="count" fill="#ffaa00" name="Times targeted" radius={[0, 6, 6, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                ) : (
                  <div className="text-center py-12 text-gray-500 text-sm">No destination data.</div>
                )}
              </CardContent>
            </Card>
            ...
            <Card className="bg-[#0f1825]/70 border-[#1a2540] backdrop-blur">
              <CardHeader>
                <CardTitle className="text-white flex items-center gap-2">
                  <Network className="w-5 h-5 text-[#00ccff]" />
                  Top Targeted Ports
                </CardTitle>
                <p className="text-xs text-gray-500 mt-1">
                ...
                {analytics.topTargetedPorts && analytics.topTargetedPorts.length > 0 ? (
                  <ResponsiveContainer width="100%" height={260}>
                    ...
                    >
                      ...
                      <Tooltip content={<DarkTooltip />} />
                      <Bar dataKey="count" fill="#00ccff" name="Attacks" radius={[0, 6, 6, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                ) : (
                  <div className="text-center py-12 text-gray-500 text-sm">No port data.</div>
                )}
              </CardContent>
            </Card>
          </div>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {folded.map(({ forward, reverse }, i) => (
                        <tr key={i} className="border-b border-[#1a2540]/40 hover:bg-[#1a2540]/30">
                          <td className="py-2 pr-4 font-mono text-gray-200">{forward.src}</td>
                          <td className="py-2 pr-4 text-gray-500">→</td>
                          <td className="py-2 pr-4 font-mono text-gray-200">{forward.dst}</td>
                          <td className="py-2 pr-4">
                            <span
                            ...
                            {reverse && (
                              <span
                                className="ml-2 inline-flex items-center gap-1 px-1.5 py-0.5 rounded border text-[10px] font-mono text-gray-500 border-[#1a2540] bg-[#1a2540]/30"
          ...
          <h2 className="text-sm font-semibold text-gray-300 uppercase tracking-wide mb-3 flex items-center gap-2">
            <Clock className="w-4 h-4 text-[#00ff88]" /> Activity Over Time
          </h2>
          {hourlyTimeline.length > 0 && (() => {
            ...
            const SeriesPill = ({ color, label, value }: { color: string; label: string; value: number }) => (
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg border bg-[#0b1220]/60"
            ...
            <Card className="bg-[#0f1825]/70 border-[#1a2540] backdrop-blur mb-10">
              <CardHeader>
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  ...
                  {/* Linear / log toggle */}
                  <div className="inline-flex rounded-lg border border-[#1a2540] overflow-hidden text-xs">
                ...
                <div className="flex flex-wrap items-center gap-2 mt-4">
                  <SeriesPill color="#00ff88" label="Total normal" value={totals.normal} />
                  <SeriesPill color="#facc15" label="Total suspicious" value={totals.suspicious} />
                  <SeriesPill color="#ff3366" label="Total malicious" value={totals.malicious} />
                  {peak.entry && (
                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-[#ff3366]/40 bg-[#ff3366]/5">
                      <Flame className="w-3.5 h-3.5 text-[#ff3366]" />
                      <span className="text-[11px] text-gray-400 uppercase tracking-wide">Peak hour</span>
                      <span className="font-mono text-sm text-white">{peak.entry.hourLabel}</span>
                  ...
                  {anomalies.length > 0 && (
                    <div
                      className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-[#ffaa00]/40 bg-[#ffaa00]/5"
                    ...
                    >
                      ...
                      <span className="text-[11px] text-gray-400 uppercase tracking-wide">Anomalies</span>
                      <span className="font-mono text-sm text-white">{anomalies.length}</span>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={420}>
                  <AreaChart data={chartData} margin={{ left: 8, right: 24, top: 16, bottom: 4 }}>
                    ...
                    <Area
                      type="monotone" dataKey="normal" stroke="#00ff88" strokeWidth={2}
                      fill="url(#gNormal)" name="Normal" isAnimationActive={false}
                      activeDot={{ r: 4, strokeWidth: 0 }}
                    />
                    <Area
                      type="monotone" dataKey="suspicious" stroke="#facc15" strokeWidth={2}
                      fill="url(#gSusp)" name="Suspicious" isAnimationActive={false}
                      activeDot={{ r: 4, strokeWidth: 0 }}
                    />
                    <Area
                      type="monotone" dataKey="malicious" stroke="#ff3366" strokeWidth={2.5}
          ...
          {hourlyTimeline.length > 0 && (() => {
            ...
            return (
              <Card className="bg-[#0f1825]/70 border-[#1a2540] backdrop-blur mb-10">
                <CardHeader>
                  <CardTitle className="text-white flex items-center gap-2">
                    <Flame className="w-5 h-5 text-[#ff6633]" />
                ...
                <CardContent>
                  <div className="overflow-x-auto">
                    ...
                    >
                      {/* Header row */}
                      <div className="text-[11px] text-gray-500 uppercase tracking-wide pb-2">
                        Hour
                      </div>
                      {families.map((f) => (
                        <div
                    ...
                    {/* Legend */}
                    <div className="flex items-center gap-3 mt-4 pt-3 border-t border-[#1a2540]/60 text-[11px] text-gray-500">
                      <span>Intensity (log scale):</span>
                      <div className="flex items-center gap-1">
                        {[0.1, 0.25, 0.5, 0.75, 1].map((a, i) => (
                          <div key={i} className="w-6 h-3 rounded-sm" style={{ background: `rgba(255,51,102,${a})` }} />
                        ))}
                      </div>
          ...
          <div className="grid lg:grid-cols-2 gap-8 mb-10">
            {analytics.mitreTechniqueCounts && analytics.mitreTechniqueCounts.length > 0 ? (
              <Card className="bg-[#0f1825]/70 border-[#1a2540] backdrop-blur">
                <CardHeader>
                  <CardTitle className="text-white flex items-center gap-2">
                    <Globe className="w-5 h-5 text-[#cc66ff]" />
                ...
                <CardContent>
                  <div className="grid grid-cols-[1fr_140px] gap-3">
                      ...
                      >
                        ...
                        <Tooltip
                          content={({ active, payload }) => {
                            ...
                            return (
                              <div className="bg-[#0f1825] border border-[#1a2540] rounded p-2 text-[11px] text-gray-200 shadow-lg">
                                <div className="font-mono text-[#cc66ff]">{row.id}</div>
                                <div className="text-gray-100">{row.name}</div>
                                <div className="text-gray-500">Tactic · {row.tactic || 'Unknown'}</div>
                                <div className="text-gray-400 mt-1">Flows: <span className="text-white">{row.count.toLocaleString()}</span></div>
                                <div className="text-[10px] text-gray-600 mt-1">Click bar to open attack.mitre.org</div>
                              </div>
                            );
                          }}
                        />
                        ...
                        >
                          {analytics.mitreTechniqueCounts.map((t) => (
                            <Cell key={t.id} fill={t.color ?? '#cc66ff'} />
                          ))}
                        </Bar>
                      </BarChart>
                    </ResponsiveContainer>
                    ...
                    <div className="space-y-1.5 text-[11px]">
                      <div className="text-[10px] uppercase tracking-wide text-gray-500 mb-1">Tactics</div>
                      {(analytics.mitreTacticCounts ?? []).map((t) => (
                        <div key={t.tactic} className="flex items-center gap-2">
                          <span
                            className="inline-block w-3 h-3 rounded-sm shrink-0"
                            style={{ backgroundColor: t.color ?? '#cc66ff' }}
                          />
                          <span className="text-gray-300 truncate" title={t.tactic}>{t.tactic}</span>
                          <span className="text-gray-500 ml-auto font-mono">{t.count.toLocaleString()}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {protocolData.length > 0 ? (
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={protocolData} barCategoryGap="30%">
                      <CartesianGrid strokeDasharray="3 3" stroke="#1a2540" />
                      <XAxis dataKey="name" stroke="#9ca3af" style={{ fontSize: '12px' }} tickLine={false} axisLine={{ stroke: '#1a2540' }} />
                      <YAxis stroke="#9ca3af" style={{ fontSize: '12px' }} tickLine={false} axisLine={{ stroke: '#1a2540' }} />
                      <Tooltip content={<DarkTooltip />} cursor={{ fill: 'rgba(0,255,166,0.06)' }} />
                      <Bar dataKey="count" name="Packets" fill="#00FFA6" radius={[8, 8, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                ) : (
                  <div className="text-center py-12 text-gray-500 text-sm">No protocol data.</div>
                )}
              </CardContent>
            </Card>
          </div>
