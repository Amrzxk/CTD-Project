  ...
  const handleAck = async (p: ThreatPredictionSummary | ThreatPredictionGroup, state: AckState) => {
    ...
    try {
      ...
      if (isGroup(p)) {
        ...
        if (selected?.id === p.id) {
          setSelected({ ...selected, ack_state: state, ack_at: updated.ack_at, ack_note: updated.ack_note, ackBy: updated.ackBy });
        }
      }
      setNote('');
                                  ...
                                  <div className="inline-flex flex-col items-end gap-0.5">
                                    {p.ackBy && (
                                      <span className="text-[10px] text-muted-foreground capitalize">
                                        {p.ack_state} by{' '}
                                        <span className="font-mono normal-case text-foreground">{p.ackBy}</span>
                                      </span>
                                    )}
                                    <Button
                                      size="sm"
                        ...
                        <div className="p-3 rounded-lg bg-panel/80 border border-line space-y-2">
                          ...
                          {selected.ackBy && (
                            <div className="flex items-center justify-between text-[10px]">
                              ...
                              <span className="font-mono text-foreground">{selected.ackBy}</span>
                            </div>
                          )}
                          {selected.ack_at && (
                            <div className="flex items-center justify-between text-[10px]">
                          ...
                          {/* Activity trail — who changed state, when (from the detail fetch). */}
                          {selectedDetail?.ackHistory && selectedDetail.ackHistory.length > 0 && (
                            <div className="pt-1">
                              ...
                              <ol className="space-y-1.5 border-l border-line pl-3">
                                {selectedDetail.ackHistory.map((h, i) => (
                                  <li key={i} className="relative text-[10px] leading-tight">
                                    <span className="absolute -left-[0.92rem] top-1 size-1.5 rounded-full bg-brand/70" aria-hidden />
                                    <div>
                                      <span className="font-mono text-foreground">{h.by ?? 'unknown'}</span>
                                      <span className="text-muted-foreground">
                                        {' · '}{h.from_state} → <span className="capitalize text-foreground">{h.to_state}</span>
                                      </span>
                                    </div>
                                    ...
                                    {h.note && (
                                      <div className="text-muted-foreground italic">{h.note}</div>
                                    )}
                                  </li>
                                ))}
                              </ol>
